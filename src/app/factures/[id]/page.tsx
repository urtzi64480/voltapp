...
function joursRetard(dateEcheance: string): number {
  ...
  const now = new Date();
  const diff = Math.floor((now.getTime() - ech.getTime()) / (1000 * 60 * 60 * 24));
...
export default function FactureDetailPage({ params }: { params: { id: string } }) {
  ...
  async function envoyerRelance() {
    ...
    try {
      ...
      const email = client?.email ?? "";
      const prenom = client?.prenom ?? "";
      const retard = facture.date_echeance ? joursRetard(facture.date_echeance) : 0;
      const montantDu = soldeRestant > 0 ? soldeRestant : facture.total_ttc;
      const objet = encodeURIComponent(`Relance — Facture ${facture.numero} en attente de règlement`);
      const corps = encodeURIComponent(
        `Bonjour ${prenom},\n\nSauf erreur de ma part, la facture ${facture.numero} d'un montant de ${montantDu.toFixed(2).replace(".", ",")} € reste impayée${retard > 0 ? ` depuis ${retard} jour${retard > 1 ? "s" : ""}` : ""}.\n\nJe me permets de vous en rappeler le règlement dans les meilleurs délais.\n\n👉 Consulter la facture : ${pdfUrl}\n\nCordialement`
      );
      window.open(`mailto:${email}?subject=${objet}&body=${corps}`);
    } catch (err) {
      ...
      <div className="p-4 md:p-8 max-w-3xl mx-auto">
        ...
        {estEnRetard && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-4">
            ...
            <p>Cette facture est en retard de <strong>{retardJours} jours</strong> — montant dû : <strong>{fmt(soldeRestant > 0 ? soldeRestant : facture.total_ttc)}</strong></p>
          </div>
        )}
        ...
        <div className="card card-inner mb-4 flex flex-wrap gap-4 text-sm">
          ...
          {facture.date_echeance && <div><p className="label">Échéance</p><p className={retardJours > 15 && facture.statut !== "payee" ? "text-red-600 font-semibold" : ""}>{fmtDate(facture.date_echeance)}{retardJours > 0 && facture.statut !== "payee" ? ` (+${retardJours}j)` : ""}</p></div>}
        ...
        {devisAssocie && (
          <div className="card card-inner mb-4 flex items-center gap-3">
            ...
            <Link href={`/devis/${devisAssocie.id}`} className="btn-ghost !px-3 text-xs shrink-0">Voir →</Link>
          </div>
        )}
              ...
              <div className="space-y-2 mb-3">
                {acomptes.map((ac, i) => (
                  <div key={ac.id} className="flex items-center gap-3 px-3 py-2.5 bg-blue-50 border border-blue-100 rounded-xl">
                    <div className="w-6 h-6 rounded-full bg-blue-200 text-blue-700 flex items-center justify-center text-xs font-bold shrink-0">{i + 1}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-ink-900">{fmt(ac.montant)}</p>
                      <p className="text-xs text-ink-400">
                        {fmtDate(ac.date_versement)}{ac.notes ? " · " + ac.notes : ""}
              ...
              <div className="p-3 bg-ink-50 rounded-xl border border-ink-100 space-y-3">
                ...
                <div className="flex gap-2 items-end">
                    ...
                    <div className="flex rounded-xl border border-ink-200 overflow-hidden">
                      <button onClick={() => setAcompteMode("eur")}
                        className={cn("px-3 py-2 text-xs font-semibold transition-colors",
                          acompteMode === "eur" ? "bg-ink-900 text-volt-400" : "bg-white text-ink-500 hover:bg-ink-50")}>€</button>
                      <button onClick={() => setAcompteMode("pct")}
                        className={cn("px-3 py-2 text-xs font-semibold transition-colors border-l border-ink-200",
                          acompteMode === "pct" ? "bg-ink-900 text-volt-400" : "bg-white text-ink-500 hover:bg-ink-50")}>%</button>
                      <input type="number" min="0" step="0.01"
                        placeholder={acompteMode === "eur" ? "Ex : 450" : "Ex : 30"}
                        value={acompteValeur} onChange={e => setAcompteValeur(e.target.value)}
                  ...
                  <div className="flex-1">
                    ...
                    <input type="date" className="input text-sm" value={acompteDate} onChange={e => setAcompteDate(e.target.value)} />
                  </div>
                </div>
                <div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {facture.statut === "relance" && (
            <button onClick={envoyerRelance} disabled={sendingRelance}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold transition-colors disabled:opacity-50">
              <Mail size={15} /> {sendingRelance ? "Préparation…" : "Envoyer relance"}
            </button>
          )}
          {facture.statut !== "payee" && facture.statut !== "a_envoyer" && (
            <>
              {facture.statut === "envoyee" && (
                <button onClick={marquerRelance} className="btn-ghost flex-1 justify-center">
                  <Clock size={15} /> Marquer relancée
                </button>
              )}
              <button onClick={marquerPayee} className="btn-success flex-1 justify-center">
